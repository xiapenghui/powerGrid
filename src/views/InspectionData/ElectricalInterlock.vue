<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.supplierSupportId')" width="130">
        <template slot-scope="scope">
          {{ scope.row.supplierSupportId }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.productModelOther')" width="130">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('permission.alarmItem')" width="100">
        <template slot-scope="scope">
          {{ scope.row.alarmItem }}
        </template>
      </el-table-column>
    ...
    <el-dialog title="编辑信息" :visible.sync="dialogFormVisible">
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="130px" class="demo-ruleForm">
        ...
        <div class="boxRight">
          ...
          <el-form-item label="只有当隔室的元件不带电并接地时，隔室的门或盖板才能打开" prop="elementUnchargedGround"><el-input v-model="ruleForm.elementUnchargedGround" /></el-form-item>
        </div>
      </el-form>
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          electricalEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
