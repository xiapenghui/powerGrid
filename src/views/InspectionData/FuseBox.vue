<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.continuousCurrent')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.continuousCurrent }}
        </template>
      </el-table-column>
      ...
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="130px" class="demo-ruleForm">
        <div class="bigUpBox">
          <div class="boxLeft">
            ...
            <el-form-item label="额定电流值(A)"><el-input v-model="ruleForm.ratedCurrent" /></el-form-item>
          ...
          <div class="boxRight">
            ...
            <el-form-item label="柜体序列号" prop="materialSN"><el-input v-model="ruleForm.mterialSN" :disabled="true" /></el-form-item>
            <el-form-item label="持续运行电流值(A)"><el-input v-model="ruleForm.continuousCurrent" /></el-form-item>
            <el-form-item label="熔断器附件">
              <el-upload
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="柜体序列号:">
                <span>{{ props.row.requestBody.materialSN }}</span>
              </el-form-item>
              <el-form-item label="额定电流值(A):">
                <span>{{ props.row.requestBody.ratedCurrent }}</span>
              </el-form-item>
              <el-form-item label="持续运行电流值(A)):">
                <span>{{ props.row.requestBody.continuousCurrent }}</span>
              </el-form-item>
              <el-form-item label="熔断器附件:">
...
import { rdqList, rdqDellte, rdqEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/api/kvsc/rdq/import/file',
      ...
      imgList: [], // 批量上传图片数组
      fileList: [],
      newDataImg: { id: '', imagePath: '', modelName: '熔断器' }, // 多个图片上传
      oneDataImg: { id: '', imagePath: '', modelName: '熔断器' }, // 单个图片上传或替换之前的图片
      editRow: {},
      editFileList: [],
      ...
      rules: {
        ...
        materialSN: [{ required: true, message: '请输入柜体序列号', trigger: 'blur' }]
      }
    }
  },
  computed: {},
  ...
  methods: {
    ...
    getList() {
      ...
      rdqList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          rdqEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
