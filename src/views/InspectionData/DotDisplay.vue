<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.pressureValueUn')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('permission.articulationMax')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.articulationMax }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.articulationMin')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.articulationMin }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.articulationTest')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.articulationTest }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.inspectionReportFileDDXSQ')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
...
import { ddxsqList, ddxsqDellte, ddxsqEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/api/kvsc/ddxsq/import/file',
  ...
  methods: {
    ...
    getList() {
      ...
      ddxsqList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ddxsqEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
