<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.inspectionReportFileCGTG')" width="150">
        <template slot-scope="scope">
...
import { cgtgList, cgtgDellte, cgtgEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/api/kvsc/cgtg/import/file',
      ...
      imgList: [], // 批量上传图片数组
      fileList: [],
      newDataImg: { id: '', imagePath: '', modelName: '穿柜套管' }, // 多个图片上传
      oneDataImg: { id: '', imagePath: '', modelName: '穿柜套管' }, // 单个图片上传或替换之前的图片
      editRow: {},
      editFileList: [],
  ...
  methods: {
    ...
    getList() {
      ...
      cgtgList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          cgtgEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
