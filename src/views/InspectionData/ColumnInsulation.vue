<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.SaleOrg')" width="100" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.saleOrg }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" :label="$t('permission.inspectionReportFileZZJYZ')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="130px" class="demo-ruleForm">
        <div class="bigUpBox">
          <div class="boxLeft">
            ...
            <el-form-item label="耐压值(kV)"><el-input v-model="ruleForm.pressureValue" /></el-form-item>
...
import { zzjyzList, zzjyzDellte, zzjyzEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      ...
      imgList: [], // 批量上传图片数组
      fileList: [],
      newDataImg: { id: '', imagePath: '', modelName: '支柱绝缘子' }, // 多个图片上传
      oneDataImg: { id: '', imagePath: '', modelName: '支柱绝缘子' }, // 单个图片上传或替换之前的图片
      editRow: {},
      editFileList: [],
  ...
  methods: {
    ...
    getList() {
      ...
      zzjyzList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          zzjyzEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
