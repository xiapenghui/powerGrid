...
import { jctList, jctDellte, jctEdit, allLogs } from '@/api/tenGrid'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      jctList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          jctEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
