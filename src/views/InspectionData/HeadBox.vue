<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.inspectionReportFileCTH')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
...
import { cthList, cthDellte, cthEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/api/kvsc/cth/import/file',
      ...
      imgList: [], // 批量上传图片数组
      fileList: [],
      newDataImg: { id: '', imagePath: '', modelName: '触头盒' }, // 多个图片上传
      oneDataImg: { id: '', imagePath: '', modelName: '触头盒' }, // 单个图片上传或替换之前的图片
      editRow: {},
      editFileList: [],
  ...
  methods: {
    ...
    getList() {
      ...
      cthList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          cthEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
